...
cc(cd,ad:getBackground()).get()end}end}end;aa["libraries"]={}
...
local ab=da("basaltDraw")local bb=da("utils")
local cb,db,_c,ac=math.max,math.min,string.sub,string.rep
return
function(bc,cc)local dc=cc.getObject("Container")(bc,cc)
local _d="BaseFrame"local ad,bd=0,0;local cd={}local dd=true;local __a=cc.getTerm()local a_a=ab(__a)
local b_a,c_a,d_a,_aa=1,1,false,colors.white
local aaa={getType=function()return _d end,isType=function(baa,caa)return
_d==caa or dc.isType~=nil and dc.isType(caa)or false end,getBase=function(baa)return
dc end,getOffset=function(baa)return ad,bd end,setOffset=function(baa,caa,daa)ad=caa or ad;bd=daa or bd
baa:updateDraw()return baa end,getXOffset=function(baa)return ad end,setXOffset=function(baa,caa)return
baa:setOffset(caa,nil)end,getYOffset=function(baa)return bd end,setYOffset=function(baa,caa)return
baa:setOffset(nil,caa)end,setPalette=function(baa,caa,...)
if(baa==cc.getActiveFrame())then
if(
type(caa)=="string")then cd[caa]=...
__a.setPaletteColor(type(caa)=="number"and caa or
colors[caa],...)elseif(type(caa)=="table")then
